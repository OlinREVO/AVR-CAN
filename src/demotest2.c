...
ISR(INT0_vect) {
    ...
    if (val) {
        *msg = 0b01; // turn bottom LED on
    } else {
        *msg = 0b00; // turn bottom LED off
    }
...
ISR(INT1_vect) {
    ...
    uint8_t* msg = (uint8_t*)malloc(1*sizeof(uint8_t));
    if (val) {
        *msg = 0b01; // turns middle LED on
    } else {
        *msg = 0b00; // turns middle LED off
    }
    sendCANmsg(NODE_broadcast, MSG_demoMsg, msg, 1);

    if (val) {
        *msg = 0b11; // turn bottom LED on
    } else {
        *msg = 0b10; // turn bottom LED off
    }
    sendCANmsg(NODE_demoNode1, MSG_demoMsg, msg, 1);
...
ISR(INT3_vect) {
    ...
    uint8_t* msg = (uint8_t*)malloc(1*sizeof(uint8_t));
    if (val) {
        *msg = 0b11; // turn middle LED on
    } else {
        *msg = 0b10; // turn middle LED off
    }
    sendCANmsg(NODE_demoNode1, MSG_demoMsg, msg, 1);
